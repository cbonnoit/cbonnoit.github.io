...
/**
 * Helper to delete all children from a node
 * @param node {HTMLElement} Node to delete children from
 */
 export function deleteAllChildren(node) {
  const children = [...node.childNodes]
  for (let i = 0; i < children.length; i++) node.removeChild(children[i])
}
