...
export class CoachingManager {
  ...
  _updateObjection(data) {
    ...
    document.querySelector('#objection-header').textContent = `Objection: ${data['objection'].toLowerCase()}`
  ...
  _updateCoaching(coaching) {
    ...
    } else if (prompt === PROMPT_TYPES.UPDATE_WEATHER) {
      this._updateWeather(coaching['value'])
    } else {
