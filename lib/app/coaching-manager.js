import { MIN_TO_MS, SEC_TO_MS } from "../../cfg/const.js"
import { HOST_NAME, SUBSCRIBE_COACHING_ENDPOINT } from "../../cfg/server.js"
...
export class CoachingManager {
  ...
  startSession (sessionIdentifiers) {
    ...
    const url = `wss://${HOST_NAME}/${SUBSCRIBE_COACHING_ENDPOINT}?${queryString}`
   ...
   */
  receiveData (data) {
    if (data.prompt == 'summary')
      this._updateSummary(data)
    else
      this._updatePrompt(data)
  }
  ...
  _updateSummary (data) {
    ...
    for (const [partyId, intervals] of Object.entries(data['party_to_active_intervals']))
   ...
   * @param {Array.<Array.<Number>>} intervals intervals in the form [start_time, end_time]
   */
  _updateActiveIntervals (partyId, intervals) {
    ...
    // get scale converting between time (in ms) and pixel
    // TODO: dynamically adjust x axis
    const width = container.getBoundingClientRect().width
    const maxTime = 30 * MIN_TO_MS
