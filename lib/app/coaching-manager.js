...
export class CoachingManager {
  ...
  _resize() {
    ...
    window.parent.postMessage({"type": "adjustSize", "width": element.offsetWidth + marginWidth, "height": element.offsetHeight + marginHeight}, '*');
  }
   ...
   */
  _updateSummary (data) {
    if (Object.keys(this._partyToSummary).length === 0) window.parent.postMessage({"type": "coachingLoaded"}, '*'); // only load on receiving first summary
