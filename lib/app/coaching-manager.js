...
import { getTextResource } from '../runtime.js'
...
export class CoachingManager {
  constructor(sessionIdentifiers) {
    // track state per conversation: websocket for coaching, party namer, and dom elements per party
   ...
   */
  async createDOM() {
    const coachingRoot = document.querySelector('#coach-root')
    ...
    document.querySelector('#micMenu').addEventListener('click', () => {
      ...
      console.log('coming from transcript')
      this._resize()
    })
    ...
    console.log('coming from dom creation')
    this._resize()
  }
  ...
  _addSettingsMenu() {
    ...
    for (const [id, text] of Object.entries(SETTINGS)) {
      if (!DEMO_MODE && SETTING_TO_ONLY_DEMO_MODE[SETTINGS[id]]) {
   ...
   * @param {Object} data Transcript object to add
   */
  _updateTranscript (data) {
    ...
    if (!(partyId in this._partyIdToColor)) this._partyIdToColor[partyId] = generateColor(Object.keys(this._partyIdToColor).length)
    ...
    if ((back(this._transcripts) ?? {})['partyId'] === partyId)
      ...
      this._addNewTranscript(text, partyId, startTimeMs)
    }
  }
   ...
   */
  _addNewTranscript (text, partyId, startTime) {
    ...
    // fill name when available
    this._namer.getName(partyId).then((x) => nameNode.textContent = x)
    nameNode.style.color = this._partyIdToColor[partyId]
  ...
  async connectTranscriptSocket(session) {
    ...
    const hostname = FORCE_REALTIME_HOSTNAME ?? session['realtime_hostname']
  ...
  async connectCoachingSocket(session) {
    // console.log(session)
    const queryParams = { 'session_id': session['session_id'], 'party_id': session['party_id']}
    const queryString = new URLSearchParams(queryParams).toString()
    const hostname = FORCE_REALTIME_HOSTNAME ?? session['realtime_hostname']
  ...
  _updateObjection(data) {
    ...
    for (const response of responses) {
      ...
      content.appendChild(createNode('img', {
        'src': chrome.runtime.getURL('/images/circle-right-arrow.png'),
  ...
  /**
   * Update the summary
   * @param {Object} party_to_intervals Summary data
   */
  _updateSummary (party_to_intervals) {
    // update active intervals
    for (const [partyId, intervals] of Object.entries(party_to_intervals))
  ...
  _updateActiveIntervals (partyId, intervals) {
    ...
    if (!this._partyToSummary.has(partyId)) {
      this._createPartySummary(partyId)
    }
  ...
  _updateCoaching(coaching) {
    ...
    if (!DEMO_MODE && PROMPTS_TO_ONLY_DEMO_MODE[prompt]) {
      console.log(`Demo mode only prompt received ${prompt}`)
      return
    }

    if (prompt === PROMPT_TYPES.SUMMARY) {
  ...
  _clearChecklist() {
    const checklist = document.querySelector('#checklist')
    checklist.innerHTML = ''
    const loadingAttributes = {'style': 'text-align: center; font-style: italic; font-size: 12px'}
    const textToAdd = DEMO_MODE ? 'Loading suggestions...' : 'Listening for objections'
    checklist.appendChild(createNode('p', loadingAttributes, textToAdd))
    ...
    currentStageDiv.textContent = 'TBD';
    nextStageDiv.textContent = 'TBD';
  }
  ...
  _updateUI (settingId) {
    if (Object.keys(SETTINGS).includes(settingId)) {
      ...
      console.log('coming from UI update')
      this._resize()
    }
  }
